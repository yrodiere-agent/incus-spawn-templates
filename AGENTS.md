...
| `proxy` | object | MITM proxy credential injection (see [Proxy Entries](#proxy-entries)). |
...
**Important:** Project-local tools (`.incus-spawn/tools/`) with `proxy:` entries are rejected — the proxy daemon runs as a system service and cannot see CWD-dependent tools. Place proxy-enabled tools in `~/.config/incus-spawn/tools/` or a configured search path.
